...
city_set_13 = [0,1,2,6,7,8,10,11,12,13,15,17,18]
...
class Eval():
    ...
    def Print_Every_class_Eval(self, out_16_13=False, writer=None):
        MIoU = np.diag(self.confusion_matrix) / (
                    np.sum(self.confusion_matrix, axis=1) + np.sum(self.confusion_matrix, axis=0) -
                    np.diag(self.confusion_matrix))
        MPA = np.diag(self.confusion_matrix) / self.confusion_matrix.sum(axis=1)
        ...
        writer.info('===>Everyclass:\t' + 'MPA\t' + 'MIoU\t' + 'PC\t' + 'Ratio\t' + 'Pred_Retio')
        ...
        name_classes_eval = [name_classes[i] for i in city_set_13] if self.city else name_classes
        ##
        for ind_class in range(len(MIoU)):
            ...
            if writer is not None:
                writer.info('===>' + name_classes_eval[ind_class] + ':\t' + pa + '\t' + iou + '\t' + pc + '\t' + cr + '\t' + pr)
            else:
